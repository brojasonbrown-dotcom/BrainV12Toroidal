...
function createDefaultMembrane(scale: ScaleIndex, lengthM: Float64): MembraneGeometry {
  ...
  // Curvature coupling = 1/φ² ≈ 0.382 — dimensionless relaxation parameter
  // [SPEC] Pulls field toward boundary; must be <1 for stability.
  const curvatureCoupling = PHI_NEG2;
...
function computeSurfaceClosure(
  ...
  // [FIX] curvature_coupling is now dimensionless (≈0.382), so factor = κ·cc
  // is also dimensionless and stable (~0.075) regardless of physical scale.
  const boundary = membrane.boundary_field;
  const cc = membrane.curvature_coupling;
  const factor = (kappa * cc) as Float64;
...
export class FractalNode {
  ...
  /** [INTEGRATED] Actual framework computation instances for real compute() integration */
  private _frameworkComputes: (
    | F8_SubPlanckian
    | F4_Geometric
    | F5_ColorMusic
    | F6_Hebrew
  ...
  /** Active memory subsystems at this scale */
  private _memory: Map<MemoryId, MemoryInstance>;

  /** [INTEGRATED] Actual memory computation instances for real compute() integration */
  private _memoryComputes: (
    | L1_Pattern
    | L2_Hebbian
    | L3_Kuramoto
    | L4_Sensory
  ...
  constructor(
    ...
    // [FIX] Validate scale index is canonical before proceeding.
    try {
      this.scale_config = scaleMapper.get_config(scale);
    } catch (err) {
      throw new Error(
        `FractalNode: invalid scale index ${scale}. ` +
          `Must be one of the 10 canonical indices: 0, 96, 120, 134, 168, 192, 235, 250, 270, 292.`
      );
    }
    ...
    // Initialize framework metadata (backward-compatible) and computation instances
    this._frameworks = new Map<FrameworkId, FrameworkInstance>();
    this._frameworkComputes = [];
    ...
    for (const fwId of this.scale_config.active_frameworks) {
      ...
      // Metadata map (backward-compatible API)
      ...
      // Real computation instance [INTEGRATED]
      const fwCompute = this._createFrameworkInstance(fwId);
      if (fwCompute) {
        this._frameworkComputes.push(fwCompute);
      }
    }

    // Initialize memory metadata (backward-compatible) and computation instances
    this._memory = new Map<MemoryId, MemoryInstance>();
    this._memoryComputes = [];
    ...
    for (const memId of this.scale_config.active_memory) {
      ...
      // Real computation instance [INTEGRATED]
      const memCompute = this._createMemoryInstance(memId);
      if (memCompute) {
        this._memoryComputes.push(memCompute);
      }
    }
  ...
  get memory(): ReadonlyMap<MemoryId, MemoryInstance> {
    return this._memory;
  }
  ...
  compute(options?: ComputeOptions): void {
    ...
    // ── [INTEGRATED] Framework Computation ───────────────────────────────────
    // Convert interleaved complex field to real-valued input for frameworks
    const realInputSize = Math.floor(fieldSize / 2);
    const realInput = new Float64Array(realInputSize);
    for (let i = 0; i < realInputSize; i++) {
      // Use magnitude as real input: |Ψ_i| = sqrt(Re² + Im²)
      const re = totalField[2 * i] ?? 0;
      const im = totalField[2 * i + 1] ?? 0;
      realInput[i] = Math.sqrt(re * re + im * im);
    }

    // Call each active framework's compute() method and integrate
    if (this._frameworkComputes.length > 0) {
      let frameworkSum = new Float64Array(realInputSize);
      for (const fw of this._frameworkComputes) {
        const fwOutput = fw.compute(realInput, realInput); // input = realInput, field_state = realInput
        // Add framework contribution (weighted by 1/φ per framework for stability)
        for (let i = 0; i < realInputSize; i++) {
          frameworkSum[i] += fwOutput[i] * (1.0 / PHI) / this._frameworkComputes.length;
        }
      }

      // Convert framework output back to interleaved complex and add to totalField
      for (let i = 0; i < realInputSize; i++) {
        const contribution = frameworkSum[i];
        // Distribute evenly between real and imaginary parts
        totalField[2 * i] = (totalField[2 * i] + contribution * Math.cos(computePhaseAngle(i, this.scale))) as Float64;
        totalField[2 * i + 1] = (totalField[2 * i + 1] + contribution * Math.sin(computePhaseAngle(i, this.scale))) as Float64;
      }
    }

    // ── [INTEGRATED] Memory Computation ──────────────────────────────────────
    // Call each active memory's store() and recall() methods
    if (this._memoryComputes.length > 0) {
      let memorySum = new Float64Array(realInputSize);
      for (const mem of this._memoryComputes) {
        mem.store(realInput);
        const memOutput = mem.recall(realInput);
        // Add memory contribution (weighted by 1/φ² per memory for stability)
        for (let i = 0; i < realInputSize; i++) {
          memorySum[i] += memOutput[i] * (1.0 / (PHI * PHI)) / this._memoryComputes.length;
        }
      }

      // Convert memory output back to interleaved complex and add to totalField
      for (let i = 0; i < realInputSize; i++) {
        const contribution = memorySum[i];
        totalField[2 * i] = (totalField[2 * i] + contribution * Math.cos(computePhaseAngle(i + 1, this.scale))) as Float64;
        totalField[2 * i + 1] = (totalField[2 * i + 1] + contribution * Math.sin(computePhaseAngle(i + 1, this.scale))) as Float64;
      }
    }

    // ── [INTEGRATED] Child Field Feedback ──────────────────────────────────
    // Parent integrates child node field states (bottom-up feedback)
    // [FIX] Map child elements element-wise instead of summing all to all.
    if (this.children.length > 0) {
      const childWeight = (1.0 / PHI) / (this.children.length + 1e-300);
      for (const child of this.children) {
        const childField = child.state.field_state;
        for (let i = 0; i < fieldSize; i++) {
          totalField[i] = (totalField[i] + childField[i] * childWeight) as Float64;
        }
      }
    }
  ...
  stabilize(options?: ComputeOptions): void {
    ...
    // Recompute coherence and energy_density after stabilization
    // [FIX] Always recompute energy_density; coherence requires previousField.
    let newCoherence = this._state.coherence;
    if (this._previousField !== null) {
      newCoherence = computeGeometricOverlap(field, this._previousField);
    }

    // Recompute energy density: mean |Ψ|² over field elements
    let energySum = 0;
    for (let i = 0; i < fieldLen; i += 2) {
      const re = field[i];
      const im = field[i + 1] ?? 0;
      energySum += re * re + im * im;
    }
    const N = (fieldLen / 2) - 1; // fieldLen = 2*(N+1)
    const newEnergyDensity = (energySum / (N + 1)) as Float64;

    this._state = {
      ...this._state,
      coherence: newCoherence,
      energy_density: newEnergyDensity,
    };
  }
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // 11. Framework & Memory Instance Factories [INTEGRATED]
  // ───────────────────────────────────────────────────────────────────────────

  /**
   * Create a real framework computation instance from its ID.
   * [INTEGRATED] — Maps metadata IDs to actual computation classes.
   */
  private _createFrameworkInstance(
    fwId: FrameworkId
  ):
    | F8_SubPlanckian
    | F4_Geometric
    ...
    | null {
    switch (fwId) {
      case "F8_SubPlanckian":
        return new F8_SubPlanckian();
      case "F4_Geometric":
        return new F4_Geometric();
      case "F5_ColorMusic":
        return new F5_ColorMusic();
      case "F6_Hebrew":
        return new F6_Hebrew();
      case "F7_Thermodynamic":
        return new F7_Thermodynamic();
      case "F1_Septenary":
        return new F1_Septenary();
      case "F9_Hypergalactic":
        return new F9_Hypergalactic();
      default:
        return null;
    }
  }
  ...
  /**
   * Create a real memory computation instance from its ID.
   * [INTEGRATED] — Maps metadata IDs to actual computation classes.
   */
  private _createMemoryInstance(
    memId: MemoryId
  ):
    | L1_Pattern
    | L2_Hebbian
    ...
    switch (memId) {
      case "L1_Pattern":
        return new L1_Pattern();
      case "L2_Hebbian":
        return new L2_Hebbian();
      case "L3_Kuramoto":
        return new L3_Kuramoto();
      case "L4_Sensory":
        return new L4_Sensory();
      case "L4+_WeightedRAM":
        return new L4p_WeightedRAM();
      default:
        return null;
    }
  }
